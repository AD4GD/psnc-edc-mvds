...
#
# Usage:
#   ./init-identity-hub --config <config-dir>
#   ./init-identity-hub --config <config-dir> --participant consumer
#   ./init-identity-hub --config config/sage-init-identity-hub
#
...
#
# See config/init-identity-hub.example/ for reference.
#
...
usage() {
    echo "Usage: $0 --config <config-dir> [--participant <name>]"
    echo ""
    echo "Options:"
...
log()  { echo "[init-identity-hub] $(date +%H:%M:%S) $*"; }
ok()   { echo "[init-identity-hub] $(date +%H:%M:%S) OK: $*"; }
fail() { echo "[init-identity-hub] $(date +%H:%M:%S) FAIL: $*" >&2; exit 1; }
