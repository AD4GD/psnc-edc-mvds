...
normalize_string_value() {
  ...
  # Remove surrounding quotes repeatedly, e.g. '""secret""' -> 'secret'.
  while true; do
    if [[ "$value" =~ ^".*"$ ]]; then
      value="${value:1:${#value}-2}"
      continue
...
{
  "did": "${PARTICIPANT_ID}",
  "sts_client_secret_alias": "${CONNECTOR_STS_CLIENT_SECRET_ALIAS}",
  "identity_hub": {
    "base_url": "${IDENTITY_HUB_ADDRESS}",
    "identity_api_url": "${IDENTITY_HUB_ADDRESS}/api/identity",
    "credentials_api_url": "${IDENTITY_HUB_ADDRESS}/api/credentials",
    "api_key": "${IDENTITY_HUB_SUPERUSER_KEY}",
    "vault_address": "${VAULT_ADDRESS}",
    "public_key_pem_path": "certs/consumer_public.pem"
  },
  "connector": {
    "management_api_url": "${PUBLIC_SCHEME}://${CONNECTOR_HOST}/api/management",
    "api_key": "${CONNECTOR_API_KEY}",
    "dsp_url": "${PUBLIC_SCHEME}://${CONNECTOR_HOST}/api/dsp"
  }
}
EOF
