...
class VaultInitializer:
    ...
    def enable_secrets_engine(
        ...
        try:
            ...
            if "path is already in use" in str(e):
                logger.warning(f"⚠ Secrets engine already enabled at '{path}'")
                return True
            return False
        finally:
            return False
    ...
    def configure_kv_engine(
        ...
        """
        try:
            self.vault.client.sys.tune_mount_configuration(
                path=mount_point,
                max_lease_ttl="0",
                default_lease_ttl="0"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("VAULT INITIALIZATION SUMMARY")
        print("=" * 60)
        print(f"Engines enabled: {len(summary['engines_enabled'])}")
