...
## Motivation
Since we are using _consumer pull_ scenario only, while transferring the data, we need some service which will be called as a callback then it will get the data ready for transfer and save it into designated data source (S3-compatible object storage such as RustFS).
