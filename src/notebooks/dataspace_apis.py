...
def create_contract_definition(
        contract_definition_id, management_url, asset_id, policy_id, default_headers):
    return requests.post(
            headers=default_headers,
            data=json.dumps({
                "@context": {
...
def get_contract_agreement_id(contract_negotiation_id, management_url, default_headers):
    return requests.get(
        headers=default_headers,
        url=f"{management_url}/v2/contractnegotiations/{contract_negotiation_id}"
    )
            ...
            "callbackAddresses": [
                {
                    ...
                    "uri": consumer_callback_backend_url
                }
            ]
        }),
